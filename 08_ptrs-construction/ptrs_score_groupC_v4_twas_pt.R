...
#
# Per-chromosome PTRS scoring for one tissue (39 GTEx bulk-tissue variant),
# TWAS P+T (p-value-thresholding) variant, called by
# `ptrs_score_v4_twas_pt.sbatch`. Identical in logic to `ptrs_score_groupC_v4.R`
# (the FOCUS-genelist variant) -- the only difference is the gene-list source,
# passed in via the `gene_list_template` argument from the sbatch driver. Here
# that template points at the LD-clumped, per-pval TWAS P+T gene lists under
# TWAS/gene_pt_new/twas_gene_lists/.
#
# For each significant gene in the chromosome, this script:
...
#
# Usage:
#   Rscript ptrs_score_groupC_v4_twas_pt.R \
...
#   gene_list_template      Path to per-(chr, tissue, pval) gene list with
#                           placeholders {chr}, {tissue}, {pval}. For tissue,
#                           {tissue} is the cased GTEx tissue label (matches
#                           arg-7). Example:
#                             .../TWAS/gene_pt_new/twas_gene_lists/chr{chr}_{tissue}_gene_list_p{pval}.txt
################################################################################
...
# Load the per-(chr, tissue) significant-gene list
gene_list_path <- resolve_template(gene_list_template, input_chrm, tissue_type, pval)
