#!/usr/bin/env Rscript
################################################################################
# ptrs_score_groupC_v4.R
#
# Per-chromosome PTRS scoring for one tissue (39 GTEx bulk-tissue variant),
# called by `ptrs_score_v4.sbatch`. Functionally identical to
# `ptrs_score_groupC_v5.R` (the 17-CT variant) after cleanup -- both workers
# are fully parameterized via 10 positional args, so the v4/v5 difference
# lives in the driver sbatch (TWAS path table, gene-list dir, tissue list,
# default cohort). The two .R files are kept as siblings for naming parity
...
#
# Usage:
#   Rscript ptrs_score_groupC_v4.R \
...
#   gene_list_template      Path to per-(chr, tissue) gene list with placeholders
#                           {chr}, {tissue}, {pval}.
#                           Examples:
#                             .../focus_geneList_17CT/fdr_sig_genes_chr{chr}_{tissue}.txt
#                             .../focus_geneList_39/chr{chr}_{tissue}_gene_list_p{pval}.txt
################################################################################
...
# Match on version-stripped gene IDs (FOCUS gene lists are unversioned)
